...
**用同一套配方（增强/损失/调度/优化器/EMA 完全一致）训练更大的主干，提升 1.07 个百分点**
（96.43% → 97.50%）。

> **但这次对比不是单变量的，因此不能把 +1.07 单独归因于架构。** 相对 ResNet-18 那一轮，
> 同时改了 5 处：主干（11.2M → 36.5M 参数）、batch（128 → 256）、学习率（0.1 → 0.2，
> 随 batch 线性缩放）、dropout（0 → 0.3）、stochastic depth（0 → 0.1）。
> 它是「更大主干 + 相应超参 + 额外正则」的**合计**效果。干净的归因需要单变量消融，
> 见 `docs/TRAINING_PLAN.md` 阶段 2。
